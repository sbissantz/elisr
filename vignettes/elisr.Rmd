...
Later on, we will talk about this inclusion criterion in greater detail. Making
a long story short: Using `overlap()` an item can appear in more than one of the
enriched fragments. In doing so, we overcome the splitting effect of the data
frame induced by `disjoint()`. These basic principles will unfold one step at a
time throughout the rest of this companion. So, stay tuned!
