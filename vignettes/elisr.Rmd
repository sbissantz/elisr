...
reveal the key strategy of an exploratory analysis using `elisr`. If you consult
the bottom-up item-selection procedure for advice, you'll often find your self
following these two steps: (1) Instruct `disjoint()` to produce a couple of
sharp, disjoint scale fragments from your specified data frame. (2) Let
`overlap()` pick up the pieces. A more detailed version of this will unfold
...
institutions and organizations (type `?trust` for more information). Because the
data frame is already part of `elisr`, we can easily access it.
...
can't see the output I am currently talking about, something went wrong. I made
`disjoint()` (and `overlap()`) actually complain about quite a number of things.
Hopefully, the provided message are sensible enough to let you cope with the
...
applied research. But if you can't encrypt the error message, hack it into
...
frame (`mrit = rbar = 0.92`). To find it, `disjoint()` (and `overlap()`) set up
a correlation matrix internally and asks for the one (correlation) that is the
highest of them all. And that's how the initial step of the scaling procedure
...
`eucomission`. As already mentioned above, both variables share the highest
...
variable in the data frame. It found `polpati`. Because `polpart`'s  correlation
with the sum score is higher than the specified stop criterion (`mrit_min =
0.55`) it was melted into the core. The new fragment is now a bundle of 3
Items: `eupalament`, `eucomisn`, and `polpati.` The same logic applies to the
inclusion of the other variables (e.g. `fedgovt` et al.) and reveals the second
step of the algorithmic procedure: Continue to collect new items from `df = trust`, until the correlation between the sum-score of the items in the fragment
and any other item is less than your pre-specified `mrit_min.`
...
> **Note**: To ensure disjointedness the function(s) exclude a variable after
...
you need to report more than two decimal places, expand the result using `digits = n` and choose whatever standard you like.
...
play around with this value too.
...
counterpart. That's what we have seen so far. But now we need to move on and
...
a possible way out of the self-induced disjointedness -- `overlap()`. That's why
`overlap()` is not only a possibility to explore how the scales reunite, but a
way to monitor how each scale evolved.
...
`overlap()` remembers the start- and endpoint you've specified  with
...
variables are a real subset of the data frame you've planed to assign as an
attribute.
...
`print()`s version of `overlap()`'s and `disjoints()` output is truly a bunch of
summary statistics applied to it (see `?print.msdf`). To put it another way,
...
it. Note that `alpha()`'s output is pretty extensive. That's way I suppressed
