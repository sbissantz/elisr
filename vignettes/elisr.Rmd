...
analysis I will stick with it for now. But feel free to play around with this
...
(which is obviously greater than our prespecefied `mrit_min = 0.55`): `mrit =
...
other item is less than your prespecefied `mrit_min.`
...
necessary correlations need to be positive and higher than your prespecefied
...
`mrit_min` serves you more as a dispersion criterion. What does that means in
practical terms? Well, deregulate `overlap()`'s `mrit_min` -- to allow
`overlap()` to soak up additional items from your data set. In the following
snippet I stick with `mrit_min = 0.4` (because it produced satisfactory results
in prior analysis). But feel free to noodle around with this value. This helps
you to fully understand `mrit_min`'s behavior within `overlap()`.
...
part of `scl_1` it still meets the preset inclusion criterion in `scl_2`. So
