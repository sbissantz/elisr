...
linear relationship between the sum score of the items in a fragment at some
specific point of the scale development process and the item which is considered
for admission. Hence, the output represents the construction from a bottom-up
...
of choice. Second, hand the proposed scale over to `alpha()`. That is basically
it. Since `msdf` already offers a scale to analyze, let's stick with it. Note
that `alpha()`'s output is pretty extensive. That is why I suppressed the
result. But if you run the code yourself, you will see the result in full
length.
