...
lais out with this approach, you might want to take another look in the cited
paper as well. There is at least one article in prgress which is not mentioned
right now. I will update the list as soon as possible. Let's summarize the
introduction with the most important features. After that we dive deeper into
the technical details to make you maximal comfortable when using `elisr`.
...
With a typical case in mind, the practical difference between the two is: (1)
...
`?trust` for more details. Because the data frame is part of `elisr`, we can
access it easily. 
...
Let's put things into practice. In the first snippet, as I already said, we will
instruct `disjoint()` to produce a couple of sharp fragments. Do that by setting
a relatively high value for the marginal corrected item total correlation
`mrit_min`. To cut things short, I'll use `.55` (mostly because this value
produced satisfactory results in prior analyses -- a more detailed explanation
on how to use this value you will find below).
...
handle if you enable the `negative_too` option.
...
A minor issue of `elisr` so far, is that it can't cope with items measured on
...
A final note on what `elisr` leaves you with. Well, what you get from applying
...
```{r}
core <- trust[c("tv", "bundtag", "fccourt")]
(msdf <- disjoint(df = core, mrit_min = 0))
attributes(msdf)$df <- trust
(msdf_fin <- overlap(msdf, mrit_min = .3))
```
