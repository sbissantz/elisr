...
internally defined inclusion criterion -- namely, that the sumscore of the whole
...
To conclude, let's take a final look on both fragments. If we remind ourselves
on the description of `disjoint()`s task (to build disjoint scale fragments),
...
which overlaps in these two fragments. To assure that, I made `disjoint()`
exclude each variable after assigning it to a given scale fragment. In advance
of the upcoming section keep this result in mind.
...
omitting tons of decimal places does not mean that the result is accurately
rounded to the specified ones. Anyway, I decided to avoid any rounding in the
results. The reason for that is R's (as I find it, deeply confusing) rounding
standard  -- IEC 60559 (see `?round`). So, if you get into the situation where
you need to report more than two decimal places, expand the result using
`digits=n` and choose whatever standard you like.
...
bring the result on the desktop. I will stick with `mrit_min=.4` because this
value produced satisfactory results in prior analyses.
...
basic fragments from `disjoint()`. But what happened inside the machinery? Well,
...
counterpart as long as the correlation between the sum score of all items and
any other item is less than the specified `mrit_min` (reminder:`mrit.min=.4`).
To put it another way, `overlap()` takes the disjoint fragments as its basis
when trying to extend any of them. That's why the default option is called
`overlap_with="fragments"`. Additionally, `overlap()` provides the option to
choose only the highest positively correlating pair of each scale fragment
...
fundament. Anyway, `overlap()` soaked up 6 additional items from its counterpart
(`police`, ` newsppr`, `tv`, `uni`, `munadmin` & `healserv`). What happened here
