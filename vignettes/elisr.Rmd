...
not only grasp what's going on under the hood, but also encounter some benefits
of this procedure. Note however, that this is mainly thought as a manual. For a
...
analysis in the last section. Alright, let's ease up and dive now deeper into
some fiddly technical details.
...
the algorithm excludes those in a given fragment. Later on, we will talk about
this inclusion criterion in greater detail. Making a long story short: Using
...
`disjoint()`. These basic principles will unfold on step at a time throughout
the rest of this companion. So, stay tuned!

But let me tempt you with this; the paragraph above did already provide insight
into the key strategy of any exploratory analysis using `elisr`'s bottom up item
selection procedure: (1) Instruct `disjoint()` to produce a couple of sharp,
disjoint scale fragments from your specified data frame. (2) Let `overlap()`
pick up the pieces; i.e. enrich the disjoint fragments by considering those
items that are highly correlating with the (sum score of the given) fragment. A
more detailed explanation goes along with the following examples.
...
information. Because the data frame is already part of `elisr`, we can
easily access it.
...
Great, let's get the machinery working. In the following snippet, we will
instruct `disjoint()` to produce a couple of sharp fragments. Therefore, we set
a high value for the marginal corrected item-total correlation `mrit_min`.
Because `.55` produced satisfactory results in some prior analyses I will stick
with it for now. But feel free to play around with this value. A more detailed
explanation on how to systematically use `mrit_min`, you will find in one of the next chapters.
...
The table in the output shows summary statistics of a multiple scaled data frame
(`msdf`). Think about `msdf`'s generally as named `list`'s extended with a
couple of attributes for internal computational reasons (type`attributes(msdf)`
for more details). If you can't see the output I am currently talking about,
something went wrong. I made `disjoint()` (and `overlap()`) actually complain
about quite a number of things. Hopefully, the provided message are sensible
enough to let you cope with the particular obstacle. So if a brilliant oranges
shines out of your console read the message carefully (and try to follow
instructions). My first guess would always be typo. These little goblins are
nasty everyday companions in applied research. But if you can't encrypt the
error message, hack it into Google. Done? Alright, then we can go over to the
output section and learn something about `disjoint()`s machinery.
...
core of `scl_1`. The core highlights the first step of the scaling procedure --
a two-item scale. As we can see, the core of `$scl_1$ contains the two variables
that share the highest positive linear association in the data frame (`trust`).
To find it, `disjoint()` (and `overlap()`) sets up a correlation matrix and asks
for the one (correlation) that is the highest of them all. And that's how the
initial step of the scaling procedure ends, by finding the highest correlating
pair.

> **Note**: If multiple pairs share the same positive linear relationship
`disjoint()` will always choose the first one in order.

It is important to note, that in search of the highest correlation neither
...
In the prior analysis, however, everything ran smoothly. So let's put this result back on the screen and refocus on the algorithm again. We were looking for the highest correlation which sets up the core. To simplify the forthcoming
procedure, remember our previous `mrit_min` -- `0.55`.

```{r}
msdf
```
In the first line of the output we can see that the core consists of `eupalmnt`
and `eucomission`. Both share the highest correlation (which is obviously
greater than our pre-specified `mrit_min = 0.55`): $mrit = 0.92$. But once the
core was found, what happens to `$scl_1`? First, `disjoint()` added up the core
items what created a sum-score. Subsequently, `disjoint()` watched out for the
highest correlation between this sum-score and another `trust` variable in the
data frame. It found `polpati`. `polpart`'s  correlation with the sum score is
still higher than the stop criterion (`mrit_min = 0.55`) thus it is melted into
the core. The new fragment is now a bundle of 3 Items: `eupalament`, `eucomisn`,
and `polpati.` The same logic applies to the inclusion of the other variables
(e.g. `fedgovt` et al.) and reveals the second step of the algorithmic
procedure: Continue to collect new items from `df = trust`, until the
correlation between the sum-score of the items in the fragment and any other
item is less than your pre-specified `mrit_min.` 

```{r}
msdf
```

Let's now focus on `$scl_2`. `$scl_2` should raise the question, why there is
another two-item scale. First, it seems as if the stop criterion did fail. Look
at the output yourself. The `mrit` value at the end of `$scl_1` is `0.58`. But
at the beginning of `$scl_2` `mrit` is `0.67`. What is going on here? Well, the
algorithm forces `disjoint()` to build (and enrich) new fragments, like `$scl_1$
as long as there are remaining items in your data set (`df`). More specifically,
the algorithm stops only if (a) there is no variable that meets the inclusion
criterion, or (b) it soaked up the last remaining variable in the data frame. 

In addition, (a) holds for the construction process of new fragments too. But
...
minimum value. That betrays the last unique step of the disjoint scaling
procedure: If the algorithm detects some remaining items in the data set that
meets its specific requirements, it starts over again. Now we know why 
...
internally defined inclusion criterion -- namely, that the sum-score of the
whole fragment is greater than the pre-set $mrit_min$. As a consequence, no
other variable was considered to be merged into `{eupalmnt, eucomisn}`. 
...
`euparlament` to `tv`) is present in either `$scl_1` or `$scl_2` exactly once.
To put it another way, there is no intersecting variable which overlaps in these
two fragments.

> **Note**: To ensure disjointedness the function(s) exclude the variable after
assigning it to fragment. In advance of the upcoming section keep this result in
mind.
...
Thus, `msdf` is a shortcut for `print(msdf)`. But because I wrote a custom
function to print objects of the eponymous type, `print()` actually passes the
work on to `print.msdf()`. To get to the point, `print()` owns a nice way to
print the result to a specified number of digits, I equipped `print.msdf()` with
that feature as well. The default is set to `2`. However, be aware of the fact
that omitting tons of decimal places does not mean the result is accurately
rounded to the specified ones. Anyway, I decided to avoid rounding in the
results. The reason therefor is R's (as I find it, deeply confusing) rounding
standard -- IEC 60559 (see `?round`). So, if you stumble upon a situation where
you need to report more than two decimal places, expand the result using `digits
= n` and choose whatever standard you like.
...
back on the desktop. In the following snippet I will stick with `mrit_min = .4`
because it produced satisfactory results in prior analyses. But feel free to
...
`overlap()` took the disjoint fragments as its basis when trying to extend each
...
Al right, let's pull it all together now. From the last two outputs you might
...
applying its underlying algorithm to multiple disjoint fragments, considering
the according counterpart for their extension. That's what we have seen so far.
But now we need to move on and further discuss emergence of the duplicates
themselves.
...
that the scales could actually replicate because they were able to do so. To put
it another way, the procedure doesn't require the fragments to be different.
Although `disjoint()` smashes the induced list of variables, `elisr` introduces
a possible way out of the self-induced disjointedness -- `overlap()`.
That's why `overlap()` is not only a possibility to explore how the scales
reunite, but a way to monitor how each scale evolved.
...
### Further anlysis of the trust items 

In everyday research, one will usually stumble upon reversed items. Such
variables are broadly used to diminish response bias and reveal themselves in
general through a negative correlation in the correlation matrix. Because there
are no reversed variables in `trust` we need to make up an artificial one.
...
we can start all over again using our all in one snippet. The only thing we need
to add is `negative_too = TRUE` and specify the start- and endpoint of the scale
(as a two element vector`c(1,7)`). But before we start, a word on the upcoming
gambit: First, we should try replicate the results from the previous analysis to
...
The output is as expected. Now, we simply stick the result (stored in `d`) into
`overlap()`. But please tell `overlap()` as well, it should take reversed
variables into account (`negative_too = TRUE`). For convenience, `overlap()`
remembers the start- and endpoint you've specified  with `disjoint()` (see
`attributes(d)$sclvals`). In line with the explanations above, we expect
`overlap()` to apply the algorithm to `trust`, thereby reverse `uni` internally,
and send a reverse message.
...
There are now 3 different types of scales which `disjoint()` and `overlap()` can
...
the modified object over to `overlap()` and that's it. If you're interested in
...
the data frame, its counterpart is empty. In other words, there are no items to
...
internal security measures. So assure that at least the pre-determined variables
...
in the process. `print()` now passes the work on to the `print.msdf()`, which
...
you need to understand that what you see is *not* what you get. `print()`s
version of `overlap()`'s and `disjoints()` output is actually a bunch of summary
statistics applied to it (see `?print.msdf()`). To put it another way, the
actual outcome is hidden by this internal printing mechanism. But you can break
through this process by addressing one of `msdf`'s elements directly. This
reveals the internally sorted (and in this case partially reversed, plus
extended) data list. To keep things organized, the following output shows only
the first six values (`head(x)`) of `msdf`'s component one: `$scl_1`.

```{r}
head(msdf$scl_1)
```

> **Note**: There is one tiny thing I want to point out here. Look at `tv`. It
is reversed now. Hence, the reversing process was successful. If you want to
prove that, just compare the outcome to previous results in this section and you
will see that they are identical (type: `overlap(disjoint(trust, mrit_min=.55), mrit_min=.4`). In a nutshell: `disjoint()` and `overlap()` both keep the
reversed form of their variables. This behavior is thought to make further
analysis as convenient as possible. Let's move on, discussing why.
...
of your choice. Second, hand the proposed scale over to `alpha()`. That's
basically it. Since `msdf` already offers a scale to analyze, let's stick with
it. Note that `alpha()`'s output is pretty extensive. Therefore, I suppressed
...
As we've seen throughout the manual, an analysis with `elisr` often provides
multiple suggestions. We should work out a solution for this scenario, too. I
usually hack the result of `overlap()` in `lapply()` and let `alpha()` do its
job on all components of the modified list. But before we start, please note
once more, that the full result will be present only if you run the code
yourself.
