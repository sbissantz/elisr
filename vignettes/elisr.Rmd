...
those that are already built into a fragment. We will talk about the inclusion
criterion later in much greater detail. To get to the point: Using `overlap()`
an item can appear in more than one of the enriched fragments. In doing so, we
overcome the splitting effect `disjoint()` induces. These basic principles
unfold one step at a time throughout the rest of this companion. So, stay tuned!
...
the machinery working. First, we inspect `disjoin()`  and then move on to
`overlap()`.
...
`disjoint()` to incorporate such items. Al rightt, let's put things into
practice. Because `mrit_min = 0.55` produced satisfactory results in prior
analyses I will stick with it for now. But feel free to play around with this
value (to fully understand its behavior).
...
The above output shows summary statistics of a multiple scaled data frame
(`msdf`). Think of `msdf`s as named `list`s extended with a couple of attributes
for internal computational reasons (see `attributes(msdf)`). If you cannot see
the output I am currently talking about, something went wrong. If everything ran
smoothly you can skip the **Help & Advice** section to continue.

> **Help & Advice**: Okay, something went wrong. Fist of all,`disjoint()` (and
...
generally as fragments. Now take a closer look at `eupalmnt`, `eucomisn` (read
as: EU Parliament & EU Commission). This comma-separated pair of two items in
the first line is the core of `scl_1`. The core highlights the result of the
first step of the scaling procedure -- a two-item scale. As we can see, the core
of `scl_1` contains the two variables that share the highest positive linear
association in the data frame (`mrit = rbar = 0.92`). To find it, `disjoint()`
(and `overlap()`) sets up a correlation matrix internally and asks for the one
...
`msdf`'s attributes as a remainder (type `attributes(msdf)`)
...
(which is obviously greater than our pre-specified `mrit_min = 0.55`): `mrit =
...
`mrit = 0.67`. What is going on here? Well, the algorithm forces `disjoint()`
...
greater than the pre-set `mrit_min`). As a consequence, `disjoint()` did not
...
disjointedly built fragments. That is the job ob `overlap()`. Therefore, we
first need to learn about `mrit_min` (again). This will opens up `overlap()`'s
hood. After that we will continue with a closer look inside its machinery.

#### mrit_min (again)

Let's resume and try to overlap the disjointedly built fragments. To do that, we
...
into action. But before we start, just a word on the stop criterion.

With `disjoint()` we use a high value for the stop criterion to smash the scale.
Now we use it an opposite way -- to soak up up additional items from (what we
will later call) the counterpart. Leave aside the counterpart for now. Remember
the description of `mrit_min` instead. `mrit_min`is the definition of a
fragment's lower boundary and ranges between 0 and 1. This is also true for
`overlap()` (type `?overlap`). What changes for the additional `mrit_min` is, it
defines the limit under which you are not be willing to accept an item to
*complete* an emerging *scale*. To make this distinction clear, you can think of
`overlap()`'s `mrit_min` as an inclusion criterion whereas `disjoint()`s
`mrit_min` serves you more as a stop criterion. What does that means in
practical terms? Well, deregulate `mrit_min` -- to allow `overlap()` to soak up
additional items from your data set. In the following snippet I stick with
`mrit_min = 0.4` because it produced satisfactory results in prior analyses. But
feel free to noodle around with this value. This helps you to fully understand
`mrit_min`'s behavior within `overlap()`.
...
scales did actually replicate. If you want to know why, consult the paper in the
reference section.
...
mosdf
```
##### Multiple memberships -- dublicates across scales
...
reuniting often goes hand in hand, therefor we should learn how to combine the 
use of `disjoint()` and `overlap()` next.
...
```
`disjoint()` did not reverse an item? Right, that gambit was a bit unfair. But
