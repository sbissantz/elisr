...
#'   given scale need to be expanded. Therefore, \code{ovlp_nci} uses each
#'   component of the second list (\code{ebscls}) on a step by step base. In a
#'   particular case this happens as follows: While the absolute value of the
#'   correlation of a remaining item in the scale (with the sum score) is
#'   greater than the pre-specified lower bound, the scale is enlarged with the
#'   according item (from \code{ebscls}). After one overlap \code{ovlp_pci}
#'   moves on to the next scale.
#'
